...
export default function FeaturedPartners() {
    return (
        <section className="w-full py-14 items-center bg-slate-100 flex flex-col justify-center mt-14 px-4 md:px-6 lg:px-8 xl:px-0">
            <div className="w-full max-w-7xl gap-12 flex flex-col items-center">
                ...
                <div className="grid w-full md:grid-cols-4 grid-cols-2 lg:grid-cols-6 gap-6">
                    {partners.map((partner, index) => (
                        <Link
                            key={index}
                            href={partner.href}
                            className="p-6 border border-gray-300 rounded-lg flex items-center hover:scale-105 hover:shadow-md hover:shadow-primary-50 transition-all bg-white/40 hover:bg-white duration-300 justify-center flex-col gap-2 group"
                        >
                            <Image
                                src={partner.image}
